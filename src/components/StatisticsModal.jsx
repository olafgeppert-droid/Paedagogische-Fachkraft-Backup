...
const StatisticsModal = ({ allStudents, allEntries, onClose }) => { // Akzeptiert alle Studenten und Einträge
        ...
        <div className="modal-overlay">
            <div className="modal" style={{ maxWidth: '700px', width: '90%' }}>
                ...
                <div style={{ padding: '1rem', maxHeight: '70vh', overflowY: 'auto', lineHeight: '1.5' }}>
                    ...
                    <p><strong>Durchschnittliche Einträge pro Schüler:</strong> {stats.averageEntriesPerStudent}</p>
                    <hr/>
                    ...
                    {stats.sortedSubjects.length === 0 ? <p>Keine Daten vorhanden.</p> : (
                        <ul>
                            {stats.sortedSubjects.map(([subject, count]) => (
                                <li key={subject}>
                                    <strong>{subject || 'Ohne Angabe'}:</strong> {count} Einträge ({((count / stats.totalEntries) * 100).toFixed(1)}%)
                                </li>
                            ))}
                        </ul>
                    )}
